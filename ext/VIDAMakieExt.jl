...
function _triptic(
        image::SpatialIntensityMap, θ::ComradeBase.AbstractModel;
    ...
    lines!(ax3, (Xitr), parent(dataim)[:, imin], color = :cornflowerblue)
    lines!(ax3, Yitr, parent(dataim)[jmin, :], color = :red)
    ax3.yaxisposition = :right
    ax3.ylabel = "Profile"
    lines!(ax3, (Xitr), parent(fimg)[:, imin], color = :cornflowerblue, linestyle = :dash)
    lines!(ax3, Yitr, parent(fimg)[jmin, :], color = :red, linestyle = :dash)
