...
function make_initial_templates(templates...)
    ...
    lowers_const = merge(lowers, (c = (floor = 1.0e-6,),))
...
function make_initial_template_mring(N::Int, M::Int, stretch)
    ...
    if stretch
        mring = @inline function (θ)
            mod = sqrt(1 - θ.τ)
            return modify(
                CosineRing(θ.σ0 / θ.r0, θ.σ ./ θ.r0, θ.ξσ .- θ.ξτ, θ.s, θ.ξs .- θ.ξτ),
        ...
        mring = @inline function (θ)
            return modify(
                CosineRing(θ.σ0 / θ.r0, θ.σ ./ θ.r0, θ.ξσ, θ.s, θ.ξs),
                Stretch(θ.r0),
                Shift(θ.x0, θ.y0)
            )
        end
...
function combine_symbol(a::Symbol, b::NamedTuple{N, T}) where {N, T}
    k = keys(b)
    v = values(b)
    return ntuple(i -> combine_symbol(Symbol(a, "_", k[i]), v[i]), length(k))
end
